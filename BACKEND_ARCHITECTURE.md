# OptiStock: архитектура backend

Статус: исходный расширенный проект архитектуры. После уточнения пользователя реализуется MVP на существующих тестовых файлах; новые данные не ожидаются. Актуальная реализация и принятые упрощения описаны в [MVP_IMPLEMENTATION.md](MVP_IMPLEMENTATION.md) и [README.md](README.md); они имеют приоритет над первоначальным проектом ниже. В MVP очередь находится в PostgreSQL, файлы — в Docker volume, доступ — по индивидуальным API-ключам. RabbitMQ/Celery, S3 и OIDC ниже описывают дальнейшее развитие, а не уже работающие компоненты.

Название продукта: **OptiStock**. Разбор предоставленного документа со структурой проекта и сравнение с LEAFIO, Datup, REMIRA, SAP и Kinaxis — `PRODUCT_REVIEW.md`. Статусы из PDF относятся к первоначальному разбору; текущая готовность определяется кодом и проверками MVP.
