...
test('real procurement agent, persisted run, source explanation, approval and CSV', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'Запустить агента', exact: true })).toBeEnabled({
    timeout: 30000,
  })
  await page.getByRole('button', { name: 'Запустить агента', exact: true }).click()
  await expect(page.getByText('Расчёт завершён. Следующий шаг — проверка менеджером.')).toBeVisible({
    timeout: 120000,
  })
  ...
  await dialog.getByRole('button', { name: 'Save 1 changes' }).click()
